...
export async function updateCompanyAction(formData: FormData) {
  ...
  const rawSlug = getTrimmedString(formData, "slug");
  const slug = normalizeCompanySlug(rawSlug);
  const trackingEnabled = isChecked(formData, "trackingEnabled");
  ...
  const { error: updateError } = await supabase.from("companies").update({ name, slug, tracking_enabled: trackingEnabled }).eq("id", companyId);
