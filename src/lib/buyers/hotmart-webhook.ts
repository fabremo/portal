...
function buildPurchaseUpsert(
  ...
  return {
    ...
    product_id: normalizeNumber(payload.data?.product?.id),
...
export async function processWebhookLog(payload: HotmartWebhookPayload, webhookId: string) {
  ...
  if (payload.event === APPROVED_EVENT) {
    await processApprovedPurchase(payload, webhookId, nextCompanyId);
    return { handled: true as const };
  }

  if (payload.event === REFUNDED_EVENT || payload.event === CHARGEBACK_EVENT) {
    await updatePurchaseLifecycleEvent(payload, webhookId, payload.event, nextCompanyId);
    return { handled: true as const };
  }
...
export async function completeWebhookReprocessingSuccess(webhookId: string) {
  ...
  if (error) {
    throw new Error("Não foi possível concluir o reprocessamento do webhook.");
  }
}
